...
const PLAYER_ID_KEY = 'katana.playerId';
...
export class NetworkClient {
  ...
  /** THIS player's enemies (their own run), as the server has them. */
  get enemies(): ArrayLike<NetEnemyState> | null {
    const room = this.room;
    if (!room) return null;
    return room.state?.players?.get(room.sessionId)?.enemies ?? null;
  }

  /** The forge stock in effect, and the seconds until it turns over. */
  get forge(): { rotation: number; secondsLeft: number } | null {
    const state = this.room?.state;
    if (!state) return null;
    return { rotation: state.forgeRotation, secondsLeft: state.forgeSecondsLeft };
  }
  ...
  /** "I am on this katana's pad." The server checks the position and the price. */
  katanaPad(slot: number): void {
    const message: KatanaPadMessage = { slot };
    this.room?.send(MessageType.KatanaPad, message);
  }

  forgeBuy(rotation: number, slot: number): void {
    const message: ForgeBuyMessage = { rotation, slot };
    this.room?.send(MessageType.ForgeBuy, message);
  }
  ...
  get leaderboard(): LeaderboardSnapshot | null {
    ...
    return { wins: copy(board.wins), damage: copy(board.damage), time: copy(board.time) };
  }
  ...
  private bindRoom(room: Room<NetGameState>): void {
    ...
    $(room.state).players.onAdd((player, sessionId) => {
      ...
      $(player).pets.onAdd((pet) => {
        changed();
        $(pet).onChange(changed);
      });
      $(player).pets.onRemove(changed);
      $(player).killMasks.onChange(changed);
      $(player).trainingHits.onChange(changed);
      $(player).forgeBought.onChange(changed);
    });
