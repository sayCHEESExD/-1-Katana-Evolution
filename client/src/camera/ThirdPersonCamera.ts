import { CAMERA, type WorldCollision } from '@katana/shared';
...
export class ThirdPersonCamera {
  ...
  /** The world the camera must stay out of, and which portals are open to it. */
  private collision: WorldCollision | null = null;
  private openedStage = 0;
  ...
  /** Keep the camera out of this world's solids. */
  setCollision(collision: WorldCollision, openedStage: number): void {
  ...
  update(delta: number, speed: number): void {
    ...
    // Never inside a wall or a roof: pull in to the first solid between the
    // player and the camera at once, and ease back out once it is clear.
    if (this.collision) {
      const p = this.camera.position;
      const hit = this.collision.segmentClear(LOOK_TARGET.x, LOOK_TARGET.y, LOOK_TARGET.z, p.x, p.y, p.z, this.openedStage);
