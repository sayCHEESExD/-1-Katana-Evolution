...
const main = async (): Promise<void> => {
   ...
   */
  setBootStatus('Sharpening katanas…');
  await paint();
  ...
  setBootStatus('Summoning samurai…');
  game.loadingStep('Summoning samurai…');
  await paint();
  ...
  if (clientConfig.debug) {
    ...
    (window as Window & { __katana?: DebugHandle }).__katana = { game, loop };
  }
...
/** Shape of the dev-only `window.__katana` handle. */
