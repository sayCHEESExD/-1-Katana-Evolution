...
const BUX_PRODUCTS: readonly { sku: string; name: string; blurb: string }[] = [
  { sku: 'wins_small', name: 'Pouch of Wins', blurb: '+1,000 Wins: a new katana or a Bamboo Egg.' },
  { sku: 'wins_large', name: 'Chest of Wins', blurb: '+10,000 Wins: a serious upgrade.' },
];
...
export class BloxityPanel {
  ...
  /** Open the Bux store: the rail's Store tile. */
  openStore(): void {
    void this.openBux();
  }
  ...
  private async openBux(): Promise<void> {
    ...
    const rows = BUX_PRODUCTS.map((product) => {
      ...
      const buy = this.button('Buy', 'aoe-bux__buy', () => {
        ...
        void this.bloxity
          .requestPurchase(product.sku, { gameMode: 'katana' })
          .then((result) => {
            if (result.success) {
