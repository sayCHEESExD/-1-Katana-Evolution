 ...
 * Measured, not guessed: samurai.mp3 runs at -16.7 dBFS RMS (peaks -2.5), so
 * at 0.45 it sits near -24 dB RMS - a bed the katana cuts through.
 */
 ...
 *
 * The asset set ships the samurai music track, the katana slash, the enemy
 * death, a jump and a fall, and those are used as they are. Every other
...
const MUSIC_URL = '/audio/samurai.mp3';
...
const SAMPLE_URLS: Partial<Record<SoundName, string>> = {
  jump: '/audio/jump.mp3',
  // The supplied fall: a hard landing from a big drop - and the player's death.
  fall: '/audio/fall.mp3',
  death: '/audio/fall.mp3',
  // The supplied katana slash, for every swing of the local player.
  swing: '/audio/katana.mp3',
  // The supplied enemy death, for an enemy the local player fells.
  kill: `/audio/${encodeURIComponent('enemy death')}.mp3`,
};
...
export type SoundName =
  /** A katana cutting the air. */
  | 'swing'
  /** The blade landing on an enemy or a dummy. */
  | 'hit'
  /** An enemy falling. */
  | 'kill'
  | 'jump'
  ...
  | 'clear'
  /** A katana, forge item or egg bought. */
  | 'unlock'
  /** Something equipped. */
  | 'buy'
  | 'refuse'
