...
/**
 * The locally controlled samurai: a PREDICTION of a server-owned simulation.
 *
 ...
 * replicated figures (`setParams`), never from anything local.
 *
 * The swing is PRESENTATION: it plays at once for feel, and the server decides
 * separately whether it landed and for how much.
 */
export class LocalPlayer {
  ...
  /** The server's movement figures: run speed, jump, and which portals are open. */
  setParams(moveSpeed: number, jumpVelocity: number, openedStage: number): void {
  ...
  /** Play the death animation (true), or stand back up (false). */
  setDead(dead: boolean): void {
    this.character.setDead(dead);
  }

  setKatana(slot: number): void {
    this.character.setKatana(slot);
  }
  ...
  /** Play one swing now, facing `yaw` (or straight ahead when null). */
  swing(yaw: number | null): void {
    this.swingTime = 0;
  ...
  /** True while a swing is still playing, so a new one waits. */
  get swinging(): boolean {
    return this.swingTime >= 0 && this.swingTime < SWING.duration * 0.72;
  }
