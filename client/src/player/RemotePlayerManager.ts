import { VISIBLE_REMOTE_PLAYERS } from '@katana/shared';
 ...
 * nearest are DRAWN - their bodies, katanas and pets - so fifteen samurai with
 * three Yokai each cannot cost every client forty-five pets' worth of draws.
 */
export class RemotePlayerManager {
  ...
  /** Called for every swing a remote plays, so the scene can draw its slash. */
  ...
  add(sessionId: string, state: NetPlayerState): void {
    ...
    player.onSwing = (x, y, z, yaw, katana, backhand) => {
      // Only a remote that is being drawn gets a slash drawn.
      if (this.visible.has(sessionId)) this.onSwing?.(x, y, z, yaw, katana, backhand);
    };
