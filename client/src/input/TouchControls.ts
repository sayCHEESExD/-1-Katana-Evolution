...
/**
 * Touch controls: a virtual stick, the JUMP and SLASH buttons, and drag-to-look.
 *
...
export class TouchControls {
  ...
  constructor(look: LookSink) {
    ...
    this.attackButton.className = 'aoe-touch__attack';
    this.attackButton.type = 'button';
    this.attackButton.setAttribute('aria-label', 'Slash');
    this.attackButton.innerHTML = '<span>SLASH</span>';

    this.root.append(this.stick, this.jumpButton, this.attackButton);
    injectStyles();
  }
  ...
  attach(canvas: HTMLElement, container: HTMLElement): void {
    ...
    this.bindHold(this.attackButton, (down) => {
      this.attackHeld = down;
      if (down) this.attackPulse = true;
    });
  }
  ...
  apply(state: InputState): void {
    ...
    if (this.attackPulse) state.attack = true;
    if (this.attackHeld) state.attackHeld = true;
  ...
  private releaseAll(): void {
    ...
    this.attackPulse = false;
    this.attackButton.classList.remove('is-down');
  }
}
...
.aoe-touch__jump {
  right: calc(env(safe-area-inset-right, 0px) + 28px + var(--aoe-jump-size));
  width: calc(var(--aoe-jump-size) * 0.8);
  height: calc(var(--aoe-jump-size) * 0.8);
