...
const GAME_SLUG = 'katana-evolution';
const PORT = 2595;
...
const earn = async (room, seconds = 1.2) => {
  ...
  for (let i = 0; i < steps; i += 1) {
    room.send('move', { seq: seq++, dt: 0.05, moveX: 0, moveZ: 1, jump: false, cameraYaw: 0 });
    await sleep(50);
  }
  room.nextSeq = seq;
...
const PROGRESS_KEYS = [
  'xp', 'lifetimeXp', 'wins', 'lifetimeWins', 'rebirths', 'ownedKatanas', 'katanaSlot',
  'trainingTier', 'forgePercent', 'petsHatched', 'bestStage', 'playSeconds',
];
...
class Mongod {
  constructor(binary, dbPath, port) {
    ...
    this.dbPath = dbPath;
    this.port = port;
    this.uri = `mongodb://127.0.0.1:${port}/katana_verify`;
  }
  async start() {
...
const runSuite = async (ctx) => {
  ...
  console.log(`\n${kind}: purchases`);
  {
    ...
    const s = await joinAs(port(), { playerId: 'p_guest1', token: `valid:${acct}` });
    await waitFor(() => me(s).wins === winsBefore + 1000, 'the grant to be paid on join', 8000);
    pass('a recorded purchase is paid to the verified account on join (+1,000)');
    await waitForProfile(reader, acctKey, (p) => p.wins === winsBefore + 1000, 'account save with the grant');
    ...
    await waitFor(() => me(again).wins === winsBefore + 1000 + 10000, 'the grant to survive the restart', 8000);
    pass('a purchase recorded before a restart is paid after it (+10,000)');
    await waitForProfile(reader, acctKey, (p) => p.wins === winsBefore + 1000 + 10000, 'account save after the restart grant');
    await leaveAndWait(again);
    ctx.accountWins = winsBefore + 1000 + 10000;
  }
...
const run = async () => {
  ...
  if (process.env.ONLY !== 'mongo') {
    const dataDir = mkdtempSync(path.join(tmpdir(), `katana-json-${stamp}-`));
  ...
  if (!uri) {
    ...
    if (binary) {
      mongod = new Mongod(binary, mkdtempSync(path.join(tmpdir(), `katana-mongod-${stamp}-`)), MONGO_PORT);
  ...
  if (!uri) {
    ...
    const dataDir = mkdtempSync(path.join(tmpdir(), `katana-mongo-${stamp}-`));
