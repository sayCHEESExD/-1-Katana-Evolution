...
| --- | --- | --- | --- |
| `dev` | DEV | https://katana-evolve.dev.play.bloxity.io | wss://katana-evolve.dev.host.bloxity.io |
| `main` | PROD | https://katana-evolve.play.bloxity.io | wss://katana-evolve.host.bloxity.io |
...
- **Backend:** the Colyseus server is built from the root `Dockerfile` and pushed to
  `ghcr.io/<owner>/katana-evolve-server:<channel>-<sha>`. It is rolled with
  `POST https://legion.bloxity.io/v1/apps/katana-evolve/deploy`, using the commit SHA as the version,
  `seatCap` 15 (the room size) and `maxReplicas` 5. Legion injects `PORT`, `MONGODB_URI` and
  `BLOXITY_GAME_ID` (the server verifies Bloxity logins against it), and probes `/health`.
- **Frontend:** `client/dist` is built with that channel's WebSocket URL baked in, zipped with
  ...
  `POST https://api.bloxity.io/v1/hosting/games/katana-evolve/frontend?channel=<channel>&version=<sha>`.

One-time setup:

1. Create the game `katana-evolve` on https://hosting.bloxity.io (My Games).
2. Add the repository secret `LEGION_DEPLOY_TOKEN` (the token from My Games, behind the eye icon).
3. After the first run, make the GHCR package `katana-evolve-server` public so Legion can pull it:
   GitHub -> your profile -> Packages -> `katana-evolve-server` -> Package settings -> Change
...
```bash
curl https://katana-evolve.dev.host.bloxity.io/health
```
