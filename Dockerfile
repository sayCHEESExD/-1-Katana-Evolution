...
# monorepo and the server imports `@katana/shared` as a workspace dependency;
...
# imports require: `@katana/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@katana/shared` resolves through - deleting node_modules and
...
# hosts, injects one - and `serverConfig` falls back to 2587.
ENV HOST=0.0.0.0
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.PORT||2587)+'/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
